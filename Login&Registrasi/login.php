...
if (isset($_POST['login'])) {
  ...
  // Periksa login untuk admin
  $data1 = mysqli_query($koneksi, "select * from admin");
  
  while ($d = mysqli_fetch_array($data1)) {
      if ($user == $d['email'] && $pass == $d['pass']) {
          // Menciptakan session
          $_SESSION['login'] = $user;
          ...
          header("location:../HalamanUtamaAdmin/halamanUtama_admin.php");
          exit();
      }
  }
  ...
  while ($d = mysqli_fetch_array($data)) {
      if ($user == $d['email'] && password_verify($pass, $d['pass'])) {
          ...
          echo "<h1>Halo, kamu berhasil login!</h1>";
          echo "<h2>Klik <a href='login.php'> di sini (session2.php)</a> untuk menuju ke halaman pemeriksaan session</h2>";
          header("location:../HalamanUtamaUser/halamanUtama_user.php");
          exit();
            ...
            <div class="text-slider">
              <div class="text-wrap">
                <div class="text-group">
                  <h3>Pintu Gerbang Menuju Wisata yang Tak Terlupakan</h3>
                  <h3>Eksplorasi Tanah Penuh Kisah dan Pesona!</h3>
                  <h3>Jelajahi kuliner asli Yogyakarta</h3>
                </div>
              </div>
